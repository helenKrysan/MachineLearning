...
namespace MachineWPF.Practice4
{
    class TestModelPractice4
    {
        ...
        public HeuristicInformationCriterion HeuristicInformationCriterion { get { return _heuristicInformationCriterion; } }

        private FisherInformationCriterion _fisherInformationCriterion;

        public FisherInformationCriterion FisherInformationCriterion { get { return _fisherInformationCriterion; } }

        private EntropyInformationCriterion _entropyInformationCriterion;

        public EntropyInformationCriterion EntropyInformationCriterion { get { return _entropyInformationCriterion; } }

        public TestModelPractice4(bool isRandom) : this(isRandom, new Point(10, 10), new Point(5, 5), 5, 6)
        {
        }

        public TestModelPractice4(bool isRandom, Point maxXY, Point minXY) : this(isRandom, maxXY, minXY, 5, 6)
        {
        }

        public TestModelPractice4(bool isRandom, int partsX, int partsY) : this(isRandom, new Point(10, 10), new Point(5, 5), partsX, partsY)
        {
        }

        public TestModelPractice4(bool isRandom, Point maxXY, Point minXY, int partsX, int partsY)
        {
            ...
            _fisherInformationCriterion = new FisherInformationCriterion();
            Initialize(_fisherInformationCriterion.Precedents);
            ChangeRule(_fisherInformationCriterion, 1, 0);

            _heuristicInformationCriterion = new HeuristicInformationCriterion();
            Initialize(_heuristicInformationCriterion.Precedents);
            ChangeRule(_heuristicInformationCriterion, 1, 0);
        ...
        { 
            ...
            {
                new Rule(MinXY.X + eps, MinXY.X + ((MaxXY.X - MinXY.X) / PartsX) + eps, 'X', _class),
                new Rule(MinXY.Y + eps, MinXY.Y + ((MaxXY.Y - MinXY.Y) / PartsY) + eps, 'Y', _class)
            };

            creterion.SetPositiveNegative(rules, _class);
